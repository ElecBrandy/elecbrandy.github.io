...
### 마켓 컬리 : 왜 고객들은 마켓컬리에서 신선/냉동식품을 주문하지 않을까?
 #### 1. 딥다이브  
 ![uxresearch_01.png](./uxresearch_01.png)
#### 2. 가설도출
- 진짜로 다뤄야하는 문제를 정의
...
사실 딥다이브부터 너무 어려웠다. 아무래도 사전조사를 치밀하게 하지 않았기 때문에 내가 설정한 가설이 틀릴 것 같다는 두려움이 컸다.  
**처음에는 _제품탐색 과정에서 가격문제로 이탈_ 을 가설로 잡고 진행했다.**  
프리미엄 브랜드를 자부하는 마켓컬리의 상대적으로 높은 가격이 고객들로 하여금 검색과정에서 이탈을 일으킬 것이라고 가설을 설정하고, 조사와 IDI 질문을 준비했는데 그만 문제가 발생했다. 내 인터뷰이로 오신 분이 애초에 타 서비스를 이용 중이라 마켓컬리 이용 경험이 없는 사람이라는 것… 그러다보니 내가 준비한 질문 중 인터뷰이에게 던져서 쓸모있는 정보를 얻을 수 있을만한게 단 하나도 없었다. 물론 실무에서는 IDI 진행 전 대상자를 선정할때 정량조사를 통해 최소한의 자격을 가진 인터뷰이를 모집하겠지만… 내 케이스라면 간단한 정량조사를 통해 _최근 1개월 내 마켓컬리 이용 중 구매하지 않고 이탈한 사람_ 으로 좁히거나 해야하지 않았을까.  
**더 이상 인터뷰를 진행할 수 없어서 다른 가설을 설정해 두번째 인터뷰를 진행했다.**  
본격적인 IDI 인터뷰에서도 곤란했던 지점이 많다. "~한적 있으신가요?" 라는 질문에 "아니요" 로 대답했을 경우 인터뷰 진행이 턱 막히는 기분이 들었다. 오염되지 않은 정보를 얻기 위해서는 내 멋대로 생각하는 방식과 같잖은 예측을 버려야하는구나… 그렇지만 직접 해보니 어려웠다.  
**하나 더, 강사님이 "저는 원래 이렇게 해요. 보통 이런 식으로 행동해요" 라는 일반적인 답변은**큰 쓸모가 없다 하셨는데 인터뷰이가 "저는 타 서비스 이용 시에도 이렇게 행동합니다. (저는 원래 서치 버튼을 눌러 검색하는 편이라 서치 버튼을 바로 눌렀어요)" 라고 답변한 적이 많아 진행자가 더 세심하게 질문을 컨트롤 해야하는 것을 느꼈다. 느낀 것은 많지만, 잘 적용하고 싶지만 역시 쉽지 않다.
