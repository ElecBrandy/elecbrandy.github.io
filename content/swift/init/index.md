...
- 지정 생성자를 베이스로 만들어낸 바리에이션으로 (편의)생성자 내에서 지정 생성자를 호출하는 것이 특징!  
...
## 생성자의 상속과 재정의
- 생성자 상속… 넘모 어렵고 헷갈리지만 위에서 배운 Delegate up,down도 생각하면서 민첩하게 행동하자…
1. **상위 클래스의 생성자를 고려하자!**
...
> **실습 : 클래스 A를 상위 클래스로 삼는 <클래스 B>를 만들어보자**
...
이 경우는 클래스에서 편의 생성자에 해당하며 구조체는 클래스와 달리 편의 생성자라는 개념이 없으니 그냥 지정 생성자  
